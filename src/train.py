...
for fold, (train_idx, val_idx) in enumerate(kf.split(image_label_pairs)):
    ...
    train_generator = ImageLabelGenerator(
        train_pairs, batch_size=2, target_size=(512, 704), shuffle=True, augment=True
    )
    val_generator = ImageLabelGenerator(
        val_pairs, batch_size=2, target_size=(512, 704), shuffle=False, augment=False
    )

    # Model
    model = swin_unet(input_size=(512, 704, 3))
    model.compile(
    ...
    # Callbacks
    tensorboard_callback = tf.keras.callbacks.TensorBoard(log_dir=os.path.join(log_dir, f"fold_{fold+1}"), histogram_freq=1)
    ...
    coco_eval_callback = COCOEvalCallback(
        ...
        threshold=0.1,
        eval_interval=10   # run every 10 epochs
    )
    ...
    # Per-fold evaluation (standard compiled metrics)
    val_metrics = model.evaluate(val_generator, verbose=0)
    all_val_losses.append(val_metrics[0])       # loss
    all_val_dice_scores.append(val_metrics[2])  # dice_coef
    all_val_f1_scores.append(val_metrics[3])    # f1
    all_val_ap_scores.append(val_metrics[4])    # ap
    all_val_afnr_scores.append(val_metrics[5])  # afnr
...
# =========================
# Cross-validation summary (use nan-mean/std)
# =========================
...
# -------------------------
# Combined curves (pad histories then plot)
# -------------------------
